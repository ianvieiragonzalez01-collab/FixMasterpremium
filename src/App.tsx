...
export default function App() {
  ...
  const checkTrialStatus = () => {
    if (!user || user.isSubscribed) return { isExpired: false, daysLeft: 0, trialNotStarted: false };
    
    if (!user.trialStarted) return { isExpired: false, daysLeft: 3, trialNotStarted: true };
    ...
    return { isExpired, daysLeft, trialNotStarted: false };
  };

  const { isExpired, daysLeft, trialNotStarted } = checkTrialStatus();

  useEffect(() => {
    if (isAuthReady && user && (isExpired || trialNotStarted) && location.pathname !== '/assinar') {
      navigate('/assinar');
    }
  }, [isAuthReady, user, isExpired, trialNotStarted, location.pathname, navigate]);

  useEffect(() => {
    if (isAuthReady && user && !user.isSubscribed && user.pixPaymentInitiatedAt) {
      const hoursSincePixInitiated = differenceInHours(new Date(), new Date(user.pixPaymentInitiatedAt));
      if (hoursSincePixInitiated >= 24) {
            ...
            <nav className="flex-1 space-y-1">
              ...
              {!user.isSubscribed && (
                <div className="mt-6 px-2">
                  ...
                  >
                    <Zap size={20} className={location.pathname === '/assinar' ? "text-white" : "text-orange-500"} />
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden max-w-full">
          ...
          <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-40">
            ...
            <div className="flex items-center gap-3">
              {!user.isSubscribed && !isExpired && (
                <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-orange-50 text-orange-600 rounded-full border border-orange-100 mr-2">
                  <Clock size={14} />
                  <span className="text-xs font-bold uppercase tracking-tight">
                    {daysLeft} {daysLeft === 1 ? 'dia' : 'dias'} de teste
                  </span>
                  <Link to="/assinar" className="text-xs font-black underline ml-1 hover:text-orange-700">
                    Assinar Pro
                  </Link>
                </div>
              )}
          ...
          <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            <Routes>
              ...
              <Route path="/cliente/:id" element={(isExpired || trialNotStarted) ? <Subscription user={user} onUpdateUser={setUser} /> : <CustomerDetails user={user} />} />
              <Route path="/servicos" element={(isExpired || trialNotStarted) ? <Subscription user={user} onUpdateUser={setUser} /> : <Repairs user={user} />} />
