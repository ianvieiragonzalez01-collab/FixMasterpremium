...
export default function NewRepair({ user, onUpdateUser }: NewRepairProps) {
  ...
  const handleAiDiagnosis = async () => {
    ...
    if (isTrial) {
      ...
      const lastDate = user.lastAiDiagnosisDate?.split('T')[0];
      const count = lastDate === today ? (user.aiDiagnosesCount || 0) : 0;

      if (count >= AI_DAILY_LIMIT) {
        toast.error(`Limite diário de IA atingido (${AI_DAILY_LIMIT}). Assine o Pro para uso ilimitado!`, {
          action: {
            label: 'Assinar',
      ...
      const updatedUser = {
        ...user,
        aiDiagnosesCount: count + 1,
        lastAiDiagnosisDate: new Date().toISOString()
      };
    ...
    try {
      ...
      toast.error('Erro ao gerar diagnóstico com IA.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isTrial) {
      ...
      if (repairs.length >= REPAIR_LIMIT) {
        toast.error(`Limite de ordens de serviço atingido no teste grátis (${REPAIR_LIMIT}). Assine o Pro para continuar!`, {
          action: {
            label: 'Assinar',
            onClick: () => navigate('/assinar')
          }
