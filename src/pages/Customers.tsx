...
export default function Customers({ user }: CustomersProps) {
  ...
  const handleAddCustomer = (e: React.FormEvent) => {
    ...
    if (isTrial && customers.length >= CUSTOMER_LIMIT) {
      toast.error(`Limite de clientes atingido no teste grátis (${CUSTOMER_LIMIT}). Assine o Pro para continuar!`, {
        action: {
          label: 'Assinar',
          ...
          {isTrial && (
            <div className="bg-amber-50 border border-amber-100 px-4 py-2 rounded-2xl flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-amber-700 uppercase tracking-wider">
                Teste Grátis: {customers.length}/{CUSTOMER_LIMIT} Clientes
              </span>
            </div>
          )}
        </div>
